...
class MapRemover:
    ...
    def run(self):
        ...
        for i in trange(0, self.num_scans, p_dor["stride"], desc="Updating \u03B5_l", ncols=100):
            ...
            eph_l_new = (
                update_rate / (update_rate + (1 - update_rate)) 
            )

            anchor_eph_l.scatter_add_(0, inds.flatten(), eph_l_new.flatten())   # <-- (변경) index_put_ → scatter_add_
            anchor_counts.scatter_add_(0, inds.flatten(), torch.ones_like(eph_l_new).flatten())  # <-- (추가) 카운트 누적
            ...
            max_free_samples = 200_000
            if free_space_samples.size(0) > max_free_samples:
                sel = torch.randperm(free_space_samples.size(0), device=free_space_samples.device)[:max_free_samples]
            ...
            anchor_eph_l.scatter_add_(0, inds.flatten(), eph_l_new)  
            anchor_counts.scatter_add_(0, inds.flatten(), torch.ones_like(eph_l_new)) 
