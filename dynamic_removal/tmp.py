...
class MapRemover:
    ...
    def load(self, new_session : Session = None):
        ...
        for i in range(len(self.session_loader)):
            legacy_pcd = self.session_loader[i].get()
            tpcd = o3d.t.geometry.PointCloud.from_legacy(legacy_pcd).cuda() #바로 gpu에 올림
            self.gpu_scans.append(tpcd.point["positions"])       # CUDA tensor (Ni,3)
            gpu_pose = torch.from_numpy(self.session_loader.get_pose(i)[:3,3]).cuda()
            self.gpu_poses.append(gpu_pose)   
        logger.info(f"Converted all session to tpcd list")
